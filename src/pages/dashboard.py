"""
Dashboard OptiFlow - Page d'accueil centralisée
Affiche l'essentiel en 3 rangées : bandeau d'état, modules d'action, graphiques
"""
...
def load_dashboard_data():
    """Charge toutes les données nécessaires au dashboard en une fois"""
    db = OptiflowDB()
    data = {}
    ...
    try:
        ...
        # 2. Valeur totale du stock (en millions pour lisibilité)
        query_stock = """
            SELECT
        ...
        # 3. Top 3 alertes critiques avec calcul de perte réelle (sans doublons)
        query_top_alerts = """
            ...
            SELECT
                p.name as produit,
                CASE
                    WHEN pred.total_prevu > COALESCE(sl.quantity_on_hand, 0)
                    THEN (pred.total_prevu - COALESCE(sl.quantity_on_hand, 0)) * p.unit_price / 1000000
                    ELSE 0
                END as perte,
                CASE
                    WHEN COALESCE(vm.moy_ventes_jour, 0) > 0
                    THEN COALESCE(sl.quantity_on_hand, 0) / vm.moy_ventes_jour
                    ELSE 999
                END as jours_restants
            FROM alertes_uniques au
        ...
        # 4. Prédictions semaine avec calcul valeur à commander
        # Requête corrigée pour éviter la multiplication des lignes
        query_predictions = """
            WITH produits_semaine AS (
                SELECT DISTINCT product_id
                FROM forecasts
                WHERE forecast_date BETWEEN date('now') AND date('now', '+7 days')
            ),
            analyse AS (
                SELECT
                    ...
                    MAX(sl.quantity_on_hand) as stock,
                    MAX(p.unit_price) as unit_price
                FROM produits_semaine ps
                JOIN forecasts f ON f.product_id = ps.product_id
                JOIN products p ON p.id = ps.product_id
                LEFT JOIN stock_levels sl ON sl.product_id = ps.product_id
                LEFT JOIN alerts a ON a.product_id = ps.product_id AND a.status = 'active'
                WHERE f.forecast_date BETWEEN date('now') AND date('now', '+7 days')
                GROUP BY ps.product_id
            )
            SELECT
                COUNT(CASE WHEN severity IN ('critical', 'high') THEN 1 END) as nb_critiques,
                COUNT(CASE WHEN severity IN ('warning', 'medium') THEN 1 END) as nb_attention,
                SUM(CASE
                    WHEN total_prevu > COALESCE(stock, 0)
                    THEN (total_prevu - COALESCE(stock, 0)) * unit_price
                    ELSE 0
        ...
        # 6. Anomalies (remplacement de la précision système)
        # Considérer comme traitées toutes les anomalies où status != 'pending'
        query_anomalies_stats = """
            SELECT
                COUNT(*) as total,
                SUM(CASE WHEN status != 'pending' THEN 1 ELSE 0 END) as traitees
            FROM anomalies
        """
        ...
        # 8. Évolution alertes critiques (7 derniers jours)
        ...
        # 9. CA prévu vs réel (7 derniers jours PASSÉS)
        # CA prévu = ce qu'on avait prédit pour les 7 derniers jours
        query_ca_prevu = """
            SELECT
        ...
        # CA réel = ventes effectivement réalisées sur les 7 derniers jours
        query_ca_reel = """
            SELECT
...
def display_alert_counter(label, count, color):
    """Affiche un compteur d'alerte coloré"""
...
def main():
    ...
    # ========== RANGÉE 1 : BANDEAU D'ÉTAT GLOBAL ==========
    st.markdown("# 📊 OptiFlow Dashboard")
    ...
    with col1:
        ...
        html_alerts = ""
        html_alerts += display_alert_counter("🔴 Critiques", alerts_counts['CRITIQUE'], COLORS['CRITIQUE'])
        html_alerts += display_alert_counter("🟡 Attention", alerts_counts['ATTENTION'], COLORS['ATTENTION'])
        html_alerts += display_alert_counter("🟢 OK", alerts_counts['OK'], COLORS['OK'])
    ...
    with col2:
        st.metric(
            ...
            f"{data.get('stock_quantity', 0):.0f} unités"
        )
    ...
    with col1:
        with st.container(border=True):
            st.markdown("### 🚨 Top 3 Alertes Urgentes")
            ...
            if st.button("→ Toutes alertes", key="btn_alertes", use_container_width=True):
                st.switch_page("pages/alertes.py")

    # MODULE 2 : Prédictions Semaine
    with col2:
        with st.container(border=True):
            st.markdown("### 📈 Prédictions Semaine")
            ...
            if nb_critiques > 0 or nb_attention > 0:
                st.warning(f"⚠️ {nb_critiques} produits critiques, {nb_attention} attention")
            else:
                st.success("✅ Stocks suffisants")

            # Afficher la valeur à commander (info actionnable)
            st.markdown("**💰 À COMMANDER:**")
            ...
            if st.button("→ Détails produits", key="btn_predictions", use_container_width=True):
    ...
    with col3:
        with st.container(border=True):
            ...
            if st.button("→ Historique", key="btn_orders", use_container_width=True):
                st.info("Coming soon")

    # MODULE 4 : Amélioration Système
    with col4:
        with st.container(border=True):
            st.markdown("### 🔧 Amélioration Système")
            ...
            # Détails
            st.metric("Traitées", f"{anomalies_traitees}/{anomalies_total}")

            # Anomalies en attente
            anomalies_pending = data.get('anomalies_pending', 0)
            if anomalies_pending > 0:
                st.warning(f"⚠️ {anomalies_pending} en attente de validation")
            else:
                st.success("✅ Toutes à jour")

            if st.button("→ Valider", key="btn_amelioration", use_container_width=True):
                st.switch_page("pages/amelioration.py")

    st.divider()

    # ========== RANGÉE 3 : GRAPHIQUES DE TENDANCE ==========

    col1, col2 = st.columns(2)

    # GRAPHIQUE 1 : Évolution des alertes critiques
    with col1:
        st.markdown("### 📉 Évolution Alertes Critiques (7 jours)")
        ...
        if 'alerts_evolution' in data and not data['alerts_evolution'].empty:
            ...
            st.info("📊 Données historiques insuffisantes")
            st.caption("Le système est récent. Les comparaisons seront disponibles après 7 jours d'utilisation.")

    # GRAPHIQUE 2 : CA prévu vs réel
    with col2:
        st.markdown("### 💰 CA Prévu vs Réel (7 jours cumulés)")

        if 'ca_comparison' in data and not data['ca_comparison'].empty:
            df_ca = data['ca_comparison']

            # Calculer les totaux (déjà en millions)
            ...
            # Afficher l'écart
            if ecart_val >= 0:
                st.success(f"📈 Écart: +{ecart_val:,.1f} M FCFA ({ecart_pct:+.1f}%)")
            else:
                st.warning(f"📉 Écart: {ecart_val:,.1f} M FCFA ({ecart_pct:.1f}%)")
        else:
            st.info("📊 Données historiques insuffisantes")
            st.caption("Le système est récent. Les comparaisons seront disponibles après 7 jours d'utilisation.")
